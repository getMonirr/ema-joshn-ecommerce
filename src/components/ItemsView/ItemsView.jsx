...
const ItemsView = ({product,handleRemoveItem}) => {
    ...
    return (
      ...
      <div>
        <button onClick={() => handleRemoveItem(id)} className="rounded-full flex justify-center items-center bg-[#EB5757] bg-opacity-50 border-0 p-3 px-3 py-3">
          <FontAwesomeIcon className="text-[#EB5757] text-[27px]" icon={faTrashAlt} />
        </button>
      </div>
    </div>
    );
